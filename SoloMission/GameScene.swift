...
class Birds {
    var bird1 = SKSpriteNode(imageNamed: "seagull1_bitmap_cropped_new")
    var bird2 = SKSpriteNode(imageNamed: "seagull2_bitmap_cropped_new")
    var bird3 = SKSpriteNode(imageNamed: "seagull3_bitmap_cropped_new")
    var bird_counter = 1
    var speed: CGFloat = 0
    var z_pos: CGFloat = -1
...
class Brownie {
    var image_brownie = SKSpriteNode(imageNamed: "brownie1_bitmap_cropped")
    var image_brownie_hit = SKSpriteNode(imageNamed: "brownie2_bitmap_cropped")
...
class Frito {
    var image_frito = SKSpriteNode(imageNamed: "frito_bitmap_cropped")
    var image_frito_hit = SKSpriteNode(imageNamed: "frito_bitmap_rotated_cropped")
    var vel_y: CGFloat = 0
    var speed: CGFloat = 0
    var z_pos: CGFloat = -1
    var play_sound_allowed = true
...
class Misty {
    var image_misty = SKSpriteNode(imageNamed: "misty_bitmap_cropped")
    var image_misty_hit = SKSpriteNode(imageNamed: "misty_hit_bitmap_cropped")
    var image_misty_rotated = SKSpriteNode(imageNamed: "misty_bitmap_cropped_rotated")
    var image_misty_hit_rotated = SKSpriteNode(imageNamed: "misty_hit_bitmap_cropped_rotated")
...
class CheesyBites {
    var cheesy_bite = SKSpriteNode(imageNamed: "cheesy_bite_resized")
    var speed: CGFloat = 0
    init() {
        cheesy_bite.zPosition = z_pos_snacks
    }
}

class Paprikas {
    var paprika = SKSpriteNode(imageNamed: "paprika_bitmap_cropped")
    var speed: CGFloat = 0
    init() {
        paprika.zPosition = z_pos_snacks
    }
}

class Cucumbers {
    var cucumber = SKSpriteNode(imageNamed: "cucumber_bitmap_cropped")
    var speed: CGFloat = 0
    init() {
        cucumber.zPosition = z_pos_snacks
    }
}

class BegginStrips {
    var beggin_strip = SKSpriteNode(imageNamed: "beggin_strip_cropped")
    var speed: CGFloat = 0
    init() {
        beggin_strip.zPosition = z_pos_snacks
    }
}

class Broccolis {
    var broccoli = SKSpriteNode(imageNamed: "broccoli_bitmap_cropped")
    var speed: CGFloat = 0
    init() {
        broccoli.zPosition = z_pos_snacks
    }
}
...
class GameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        for i in 0...(num_backgrounds-1) {
            let string1 = "beach_background_slide"
        ...
        is_already_unlocked = highScoreNumberLevel2 >= unlock_level_points
...
//            brownie.image_brownie.position.x = self.size.width * CGFloat(num_screen_lengths_for_disp) * 3
    ...
    func runGameOver() {
        ...
        if gameScore > highScoreNumberLevel2 {
            let defaults = UserDefaults()
            defaults.set(gameScore, forKey: "HighScoreLevel2Saved")
        }
